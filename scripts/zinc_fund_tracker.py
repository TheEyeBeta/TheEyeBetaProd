...
def _alpaca(acct: str, path: str) -> Any:
    cfg = ACCOUNTS[acct]
    req = urllib.request.Request(  # noqa: S310 — fixed ALPACA_BASE https URL, not user-controlled
        f"{ALPACA_BASE}{path}",
        headers={
...
def _is_last_trading_day_of(period: str) -> bool:
    ...
    try:
        ...
        print(f"  period-boundary check failed, firing anyway: {exc}")
...
def generate_ai_brief(snaps: list[dict], period: str) -> str:
    ...
    for s in snaps:
        ...
        for p in sorted(s["positions"], key=lambda x: abs(x["day_pct"]), reverse=True)[:3]:
            lines.append(
                f"  {p['symbol']}: day {p['day_pct']:+.2f}%  total {p['unrealized_pct']:+.2f}%"
            )
    ...
    prompt = (
        ...
        f"Data:\n" + "\n".join(lines) + "\n\n"
        "Cover: (1) overall combined performance, (2) notable movers, "
        "(3) leverage and risk note. Be direct, no fluff, no bullet points."
    )
    ...
    for url, key, model in [
        ...
        try:
            ...
            req = urllib.request.Request(  # noqa: S310 — url is from the fixed list above
                url,
                data=payload,
...
def build_html(snaps: list[dict], period: str, ai_brief: str) -> str:
    ...
    ai_paras = "".join(
        f'<p style="margin:0 0 10px;line-height:1.65;color:#cbd5e1;font-size:14px;">{line}</p>'
...
if __name__ == "__main__":
    ...
    print(f"[{datetime.now(UTC).isoformat()}] Taking snapshots…")
