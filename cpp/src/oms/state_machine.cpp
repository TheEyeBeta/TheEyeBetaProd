...
expected<Order, TransitionError> StateMachine::transition(Order& order, const Event event,
    ...
    if (order.order_id.empty() || order.quantity <= 0) {
        return unexpected(make_error(TransitionErrorCode::IllegalTransition, order.status, event));
    }

    if (is_terminal(order.status)) {
        return unexpected(make_error(TransitionErrorCode::TerminalState, order.status, event));
    }
    ...
    case OrderStatus::Accepted:
        if (event == Event::PartialFill) {
            if (!apply_fill(order, fill_quantity, false)) {
                return unexpected(
                    make_error(TransitionErrorCode::InvalidFillQuantity, from_status, event));
            }
            return order;
        }
        if (event == Event::Fill) {
